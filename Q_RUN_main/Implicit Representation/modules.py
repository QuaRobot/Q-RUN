...
class Q_RUN(nn.Module):
    def __init__(self, input_dim=1, output_dim=1, hidden_dim=2048, num_layers=3):
        super(Q_RUN, self).__init__()
        self.embedding = nn.Linear(input_dim, hidden_dim)
        self.layers = nn.ModuleList()
        for _ in range(num_layers - 1):
            self.layers.append(Q_RUNLayer(hidden_dim, hidden_dim))
        self.layers.append(nn.Linear(hidden_dim, output_dim))
...
class PWLNN(nn.Module):
    def __init__(self, input_dim=1, output_dim=1, hidden_dim=2048, num_layers=3, segments=4):
        super().__init__()
        ...
        layers.append(nn.Linear(hidden_dim, output_dim))  # 输出层
        self.layers = nn.Sequential(*layers)

    def forward(self, coords):
        output = coords.clone().detach().requires_grad_(True)  # allows to take derivative w.r.t. input
        for layer in self.layers:
            output = layer(output)
        return output, coords
...
class FNO(nn.Module):
    def __init__(self, input_dim=1, output_dim=1, hidden_dim=32, num_layers=3):
        super().__init__()
    ...
    def forward(self, coords):
        output = coords.clone().detach().requires_grad_(True)  # allows to take derivative w.r.t. input
        output = self.linear1(output.squeeze(0)).unsqueeze(1)
        output = self.net(output)
        output = self.linear2(output).squeeze().unsqueeze(0).unsqueeze(-1)
        return output, coords
